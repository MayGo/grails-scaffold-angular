 ...
 * Copyied from
 * https://github.com/grails/grails-core/blob/3163360a1ecaa2cdd48e4dd05f1b48404fdea8d9/
...
class CustomRestfulController<T> {
	...
	@RestApiParams(params = [
			@RestApiParam(name = 'max', type = 'long', paramType = RestApiParamType.QUERY,
					description = 'Max number of Object to retrieve'),
			@RestApiParam(name = 'offset', type = 'long', paramType = RestApiParamType.QUERY,
					description = 'Retrieved objects list offset'),
			@RestApiParam(name = 'order', type = 'string', paramType = RestApiParamType.QUERY,
					description = 'Retrieved objects list order', allowedvalues = ['asc', 'desc']),
			@RestApiParam(name = 'sort', type = 'string', paramType = RestApiParamType.QUERY,
					description = 'Retrieved objects list sort')
	])
		...
		request.withFormat {
			form multipartForm {
				flash.message = message(
						...
						args: [
								message(code: "\${resourceName}.label",
								default: resourceClassName),
								instance.id
						]
				)
				redirect instance
		...
		request.withFormat {
			form multipartForm {
				flash.message = message(
						...
						args: [
								message(code: "\${resourceClassName}.label",
										default: resourceClassName),
								instance.id
						]
				)
	...
	protected void notFound() {
		request.withFormat {
			form multipartForm {
				flash.message = message(code: 'default.not.found.message', args: [message(code: '\${propertyName}' +
						'.label', default: '\${className}'), params.id])
				redirect action: 'index', method: 'GET'
			}
