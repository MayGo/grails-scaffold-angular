
import static org.springframework.http.HttpStatus.*
import grails.transaction.Transactional
import grails.rest.RestfulController
import grails.util.Holders
import grails.converters.JSON

@Transactional(readOnly = true)
class ${''}StatisticsController{

    static responseFormats = ['json']
	def grailsApplication
	
	def appInfo() {
		def metadata = Holders.grailsApplication.metadata
		Map appInfo = ['version':metadata['app.version']]
		render appInfo as JSON
	}
	
	def lastInserted(){
		def lastInsertedDomainClasses = []
		for (domainClass in grailsApplication.domainClasses) {
			def items = domainClass.clazz.findAll(sort:'id', order:'desc', max:'5'){}
			lastInsertedDomainClasses.addAll(items.collect{
				[
				'id':it.id, 
				'name':domainClass.propertyName, 
				'displayName': "\${domainClass.naturalName}", 
				'description':it.hasProperty('name')?it.name:it[domainClass.persistentProperties.first().name].toString()
				]
			})
		}
		render lastInsertedDomainClasses.sort{it.id}.reverse() as JSON
	}
	
	def totalInserted(){
		def insertedDomainClasses = []
		for (domainClass in grailsApplication.domainClasses) {
			insertedDomainClasses << ['name':domainClass.propertyName, 'displayName': "\${domainClass.naturalName}", 'count':domainClass.clazz.count()]
		}
		render insertedDomainClasses as JSON
	}
	
	def lastUpdated(){
		render [] as JSON
	}
}
